...
## TFM - Master en Big Data y Ciencia de Datos
## Directora: Nery Sofia Huerta, PhD
## Estudiante: Ing. Angel Farfán Bernal
...
# ****** Grafica de linear de cada entidad por año **********************************************
entidades_unicas <- unique(data1_long$ENTIDAD)

# Convertir 'AÑO' a factor ordenado 
T_AÑO_ENTIDAD$AÑO <- factor(T_AÑO_ENTIDAD$AÑO, levels = sort(unique(T_AÑO_ENTIDAD$AÑO)))

# Reordenar T_MES
T_AÑO_ENTIDAD <- T_AÑO_ENTIDAD[order(T_AÑO_ENTIDAD$AÑO), ]

#Convertir la columna ENTIDAD en factor para que aparezcan todos en la grafica
T_AÑO_ENTIDAD$ENTIDAD <- factor(T_AÑO_ENTIDAD$ENTIDAD, levels = sort(unique(T_AÑO_ENTIDAD$ENTIDAD)))


# HABILITAR EN EL CASO QUE SE QUIERA MOSTRAR LA LEYENDA DE X EN BASE A UNA CONDICION DEL VALOR TOTAL
#entidades_con_etiqueta <- with(T_AÑO_ENTIDAD, tapply(VALOR_TOTAL, ENTIDAD, max))
#entidades_con_etiqueta <- names(entidades_con_etiqueta[entidades_con_etiqueta > 400])
#etiquetas_personalizadas <- function(x) {
#  ifelse(x %in% entidades_con_etiqueta, x, "")
#}


# Crear un vector manualmente con las entidades que se desea mostrar en el eje X
entidades_mostrar <- c("BAJA CALIFORNIA", "CHIHUAHUA", "GUANAJUATO", "JALISCO", "SONORA","TAMAULIPAS")

# Función para mostrar solo esas etiquetas
etiquetas_personalizadas <- function(x) {
  ifelse(x %in% entidades_mostrar, x, "")
}


# Crear gráfico con escalas ajustadas
ggplot(data=T_AÑO_ENTIDAD, aes(x=ENTIDAD, y=VALOR_TOTAL, group=AÑO, color=AÑO)) +
  geom_line() +
  geom_point() +
  facet_wrap(~AÑO, scales="free_y") +  # Escala independiente para cada gráfico
  scale_x_discrete(labels = etiquetas_personalizadas) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle=45, hjust = 1, vjust=1, size = 7),
        plot.title = element_text(hjust = 0.5, size = 15, face = "bold")) +
  labs(title="Evolución de los registros por año de cada Entidad",
       x="Entidad",
       y="Total de registros") 
...
# Obtener el cluster asignado para cada entidad
cluster_assignments <- cutree(pheatmap_result$tree_row, k = 2)
# Entidades que pertenecen al cluster con menos delitos 
...
mapa_cluster_1 <- ggplot() +
  ...
  scale_fill_manual(values = colores_mapa,                     
                    labels = c("Mayor", "Menor"),  # Cambiar etiquetas de la leyenda
                    name = "Delitos") +
  theme_void() +
  labs(title = "Mapa de mayor o menor presencia de delitos") +
...
# En el siguiente bloque se excluye Extranjero y se añade la columna del cluster 
# al df original con un merge de aucerdo a la entidad, ademas se obtiene el arbol de desiciones

# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

#Convertir el resultado de clustering en un data.frame (entidades que pertenecen a cluster 1 y cluster 2)
df_clusters <- data.frame(ENTIDAD = names(cluster_assignments),
                          CLUSTER = cluster_assignments)
# Filtrar las entidades para descartar extranjero
df_clusters <- df_clusters[df_clusters$ENTIDAD != "EXTRANJERO", ]

data1_sin_extranjero <- data1[data1$ENTIDAD != "EXTRANJERO", ]
# Unir data1 con la variable de cluster: nombre de la variable "CLUSTER" 
data1_con_clusters <- merge(data1_sin_extranjero, df_clusters, by = "ENTIDAD", all.x = TRUE)
...
# Entrenar el árbol
arbol_detallado <- rpart(CLUSTER ~ AÑO + TIPO + MES + VALOR,
                         data = data1_con_clusters,
                         method = "class",
                         control = rpart.control(cp = 0.001, minsplit = 5, maxdepth = 10))

# Crear una paleta personalizada: colores diferentes para los 2 clusters
# Los clusters son numéricos (1, 2)
colores_clusters <- c("lightgray", "lightblue")  # Cluster 2 en azul claro 
names(colores_clusters) <- c("1", "2")

# Dibujar el árbol con la paleta de colorespersonalizada
rpart.plot(arbol_detallado,
           type = 2,
           extra = 104,
           box.palette = colores_clusters,
           fallen.leaves = TRUE,
           shadow.col = "gray",
           cex = 0.6)
